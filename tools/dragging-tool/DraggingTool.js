!function(global){
    ...
    global.DraggingTool = DraggingTool;
    ...
    DraggingTool.prototype.draggingStart = function(x, y){
        ...
        if (!this.selectedRegionObject){
            ...
            this.selectedRegionObject = layersManager._getRegionByPx(x, y);

            if (!this.selectedRegionObject){
                // пробуем найти регион волшебной палочкой (поиск по цвету)
                this.selectedRegionObject = RegionObject.createRegion(x, y, canvas);
                layersManager.addRegion(this.selectedRegionObject);//?????
            }

            this.selectedRegionObject.activate(canvas);

            // стереть объект
            layersManager.removeRegion(this.selectedRegionObject);//?????
